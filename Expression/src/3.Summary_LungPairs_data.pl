...
if($type and $data){	
	...
	if(lc($type) eq "pairs"){
		...
		foreach my $tumor_folder (@dires){
			if($tumor_folder !~ /^\./ and $tumor_folder ne "README.md" and $tumor_folder =~ /Lung/){
				my $saved;
		...
		open(OUT_MIR_REG,">src/DE_data_TCGA/Lung_miRNAs_regulators_$type_used\_pathway.xls") || die $!;
		...
		open(OUT_GEN_REG,">src/DE_data_TCGA/Lung_genes_regulators_$type_used\_pathway.xls") || die $!;
		...
		open(OUT_MIR,">src/DE_data_TCGA/Lung_miRNAs_DE_$type_used\_pathway.xls") || die $!;
		...
		open(OUT_PAIR,">src/DE_data_TCGA/Lung_pairs_DE_$type_used\_pathway.xls") || die $!;
		...
		if(lc($data) eq "mirnaseq"){
			$output="src/DE_data_TCGA/Lung_miRNAs_DE_Global.xls";
			...
			$output="src/DE_data_TCGA/Lung_genes_DE_Global.xls";
